	...
	*/
	...
	class Router{
		...
		private function find_route($_ROUTES, $path, &$args, &$params, &$name, &$route){
			/* if(array_key_exists(self::$ResolvedRouteCache, $path){
				list($args, $params) = self::$ResolvedRouteCache[$path];
			...
			foreach($_ROUTES as $name=>$params){
				...
				if (preg_match($route, $path, $args)) {
					if(self::resolve_controller($args, $params)){
						self::$ResolvedRouteCache[$path] = array($args, $params);
						return TRUE;
					}
				}
			}
		...
		private function resolve_controller($args, $params){
			...
			if(file_exists($controllerfile)){
				...
				if(class_exists($controllerclass)){
					...
					if(is_a($controller, 'Phyre\Controller')){
						return $controller->execute($args, $params);
					}
				}
				else{
					throw new ControllerNotDefinedException("Controller '$controllerclass' not defined in $controllerfile");
				}
			}
		}
	}
	...
	abstract class Controller{
		public static $view_engine = "Phyre\TemplateView";
		private $name;
		protected $args;
	...
	class TemplateNotFoundException extends PhyreException {}
	class ControllerNotDefinedException extends PhyreException {}
